...
def import_database(workload_name, scale_factor, explicit_database_name=None, host=None, port=None, user=None, password=None, run_id=None, connected_database_name=None):
    random.seed(0)
    ...
    generating_connector = PostgresDatabaseConnector(connected_database_name, autocommit=True, host=host, port=port, user=user, password=password)
...
def main(run_cfg):
    if len(run_cfg) == 0:
        ...
        connected_database_name = run_cfg["connected_database_name"] if "connected_database_name" in run_cfg else None
        explicit_database_name = run_cfg["explicit_database_name"] if "explicit_database_name" in run_cfg else None
        import_database(workload_name, scale_factor, explicit_database_name=explicit_database_name, host=host, port=port, user=user, password=password, run_id=run_id, connected_database_name=connected_database_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--connected_database_name', type=str, help='The database name used for the connection')
