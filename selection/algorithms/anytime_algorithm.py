...
class AnytimeAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload):
        ...
        candidates = candidates_per_query(
            ...
            # base rule 
            # candidate_generator=based_rule_indexes,
        )
        print("input dta num", sum([len(li) for li in candidates]))
        ...
        print("len(merged_candidates)", len(candidates))

        # Remove candidates that cannot meet budget requirements
        seeds = []
        filtered_candidates = set()
        for candidate in candidates:
            if not candidate.estimated_size:
        ...
        for i, seed in enumerate(seeds):
            logging.info(f"Seed {i + 1} / {len(seeds)}")
            ...
            indexes, costs = self.enumerate_greedy(
                workload, seed, current_costs, candidates_copy, math.inf, deep=0
            )
    ...
    def enumerate_greedy(
        ...
        if deep > 20:
            logging.info(f"Searching deep {deep} in {len(candidate_indexes)} indexes")
