...
class AutoAdminAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload):
        ...
        for current_max_index_width in range(1, self.max_index_width + 1):
            ...
            # logging.info(f"current_max_index_width: {current_max_index_width},  {candidates}")
    ...
    def select_index_candidates(self, workload, potential_indexes):
        ...
        for i, query in enumerate(workload.queries):
            ...
            # logging.info(f"Need to get cost candidates for {i}/{len(workload.queries)} query {query} enumerate indexes: {len(indexes)} => {indexes}")
    ...
    def enumerate_combinations(self, workload, candidate_indexes):
        ...
        # logging.debug(log_out)
        log_info = f"w_sz: {len(workload.queries)} ind_k: {len(current_indexes)} lowest cost (naive): {costs}  \tlowest cost indexes (naive): {current_indexes}"
        ...
        log_out = f"lowest cost (greedy): {costs}" + f"\tlowest cost indexes (greedy): {indexes}" + f"(greedy): number indexes {len(indexes)}"
        # logging.debug(log_out)
    ...
    def enumerate_naive(self, workload, candidate_indexes, number_indexes_naive):
        ...
        candidate_indexes.sort() # Fixed order (for easy reproducibility)
        
        # logging.info(f"exec enumerate_naive => len(candidate_indexes) {len(candidate_indexes)}")
    ...
    def enumerate_greedy(
        ...
        if best_index[0] and best_index[1] < current_costs:
            ...
            # logging.info(f"greedy Additional best index found: {best_index}")
