...
def save_workload(workload, pickle_path):
    ...
    print(f"Saved workload: {pickle_path}")
    return pickle_path
...
def gen_tpc(config):
    ...
    print(f"gen {workload_name} use config:", config)
...
def gen_imdb(cfg):
    ...
    print(f"gen {workload_name} use config:", cfg)
    ...
    workload_imdb = paramize_imdb(selected_sql_list, database_name, db_stat, times=gen_query_times)
...
def gen_workload(cfg_dict, func_map):
    for w_name in cfg_dict:
        func_map[w_name](cfg_dict[w_name])
...
if __name__ == "__main__":
    ...
    gen_workload(cfg_dict, gen_func_map)
