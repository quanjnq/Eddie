import random
import workload_gen.randoms
import workload_gen as sqlfuzz
from string import ascii_lowercase
from workload_gen.common import *
...
def select_word(src_data, type):
    ...
    length = sqlfuzz.randoms.random_int_range(len(oneword) - offset)
...
def gen_comp_num(tbl_col, cmp_op, db_stats, samll_sel):
    col_stat = db_stats[tbl_col]
    if len(col_stat["histogram_bounds"]) == 0 and len(col_stat["most_common_vals"]) == 0:
        col_stat["most_common_vals"].append(0)
        col_stat["most_common_freqs"].append(0)
    
    freq_val_list = sorted([(freq, val) for freq, val in zip(col_stat["most_common_freqs"], col_stat["most_common_vals"])], key=lambda x: x[0])
    most_val_list = sorted(col_stat["most_common_vals"])
    sel = 0.01 # 选择度界限
    ...
    comp_num = None
    if rint < 500 if samll_sel is not None else samll_sel: # 取较小的
        if cmp_op == "=":
            comp_num = (random.choice(col_stat["histogram_bounds"]) - rval) if len(col_stat["histogram_bounds"]) > 0 else (freq_val_list[0][1]-rval if freq_val_list[0][0] < sel else most_val_list[0]-rval)
        elif cmp_op == "<":
            comp_num = col_stat["histogram_bounds"][0] - rval if len(col_stat["histogram_bounds"]) > 0 else most_val_list[0] - rval
        elif cmp_op == "<=":
            comp_num = col_stat["histogram_bounds"][0] - rval if len(col_stat["histogram_bounds"]) > 0 else most_val_list[0]-rval
        elif cmp_op == ">":
            comp_num = col_stat["histogram_bounds"][-1] + rval if len(col_stat["histogram_bounds"]) > 0 else most_val_list[-1]+rval
        elif cmp_op == ">=":
            comp_num = col_stat["histogram_bounds"][-1] + rval if len(col_stat["histogram_bounds"]) > 0 else most_val_list[-1] + rval
    else:
        maxvals = []
        if len(most_val_list) > 0:
            maxvals = [freq_val_list[-1][1]]
        for freq, val in freq_val_list:
            if freq > sel:
                maxvals.append(val)
            
        if cmp_op == "=":
            comp_num = random.choice(maxvals) if len(most_val_list) > 0 else  random.choice(col_stat["histogram_bounds"])
        elif cmp_op in ["<", "<="]:
            comp_num = random.choice(most_val_list[1:]) if len(most_val_list) > 1 else  (0 if len(col_stat["histogram_bounds"]) < 4 else random.choice(col_stat["histogram_bounds"][3:]))
        elif cmp_op in [">", ">="]:
            comp_num = random.choice(most_val_list[:-1]) if len(most_val_list) > 1 else  (0 if len(col_stat["histogram_bounds"]) < 4 else random.choice(col_stat["histogram_bounds"][:-3]))
    return comp_num

# https://cassiopeia.readthedocs.io/en/v0.1.1/_modules/sqlalchemy/sql/operators.html
def where_generator(src_col, dst_col, src_stat, dst_stat, src_data, fuzzer, samll_sel=None):
    """
    ...
    if "CHAR" in srctype:
        ...
        if sqlfuzz.randoms.random_int_range(1000) < 900:
            ...
            op_num = float(Fraction(op_num_1, op_num_2))
        ...
        if str(src_col) in fuzzer.db_stats:
            comp_num = gen_comp_num(str(src_col), cmp_op, fuzzer.db_stats, samll_sel=samll_sel)
            # print(srctype, comp_num)
            if "INT" in srctype:
                comp_num = int(comp_num)
        
        candidate = None
        if cmp_op == "=":
            candidate = src_col.__eq__(comp_num)
        elif cmp_op == "<":
            candidate = src_col.__lt__(comp_num)
        elif cmp_op == "<=":
            candidate = src_col.__le__(comp_num)
        elif cmp_op == ">":
            candidate = src_col.__gt__(comp_num)
        elif cmp_op == ">=":
            candidate = src_col.__ge__(comp_num)
        # candidate = [
        ...
        op_select = random.choice(OP)
        op_num = select_number(src_stat, src_data, type="op")
        if (sqlfuzz.randoms.random_int_range(1000) < fuzzer.prob_config["extractyear"]):
            # print("extract date")
            random_year = random.choice(src_year)
            random_month = random.choice(src_month)

            candidate_year = [
                func.DATE_PART('year', src_col).__eq__(random_year),
                func.DATE_PART('year', src_col).__le__(random_year),
                func.DATE_PART('year', src_col).__lt__(random_year),
                func.DATE_PART('year', src_col).__ne__(random_year),
                func.DATE_PART('year', src_col).__gt__(random_year),
                func.DATE_PART('year', src_col).__ge__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__eq__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__le__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__lt__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__ne__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__gt__(random_year),
                # func.DATE_PART('year', src_col).op(op_select)(op_num).__ge__(random_year)
                ]
            candidate_month = [
                func.DATE_PART('month', src_col).__eq__(random_month),
                func.DATE_PART('month', src_col).__le__(random_month),
                func.DATE_PART('month', src_col).__lt__(random_month),
                func.DATE_PART('month', src_col).__ne__(random_month),
                func.DATE_PART('month', src_col).__gt__(random_month),
                func.DATE_PART('month', src_col).__ge__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__eq__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__le__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__lt__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__ne__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__gt__(random_month),
                # func.DATE_PART('month', src_col).op(op_select)(op_num).__ge__(random_month)
                ]
            if (sqlfuzz.randoms.random_int_range(1000) < fuzzer.prob_config["extractmonth"]):
                return combine_condition(random.choice(candidate_year), random.choice(candidate_month),
                                  random.choice(CONJ))
            else:
                return random.choice(candidate_year)

        candidate = [
            src_col.__eq__(str(random_date)),
            src_col.__le__(str(random_date)),
            src_col.__lt__(str(random_date)),
            src_col.__ne__(str(random_date)),
            src_col.__gt__(str(random_date)),
            src_col.__ge__(str(random_date))]
        # .filter(extract('year', Foo.Date) == 2012)

        return random.choice(candidate)
    else:
        return src_col.__eq__(str(random.choice(src_data)))
